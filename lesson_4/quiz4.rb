...
loop do #iterating over mailing_campaign_leads to capitalize full names
  break if counter == mailing_campaign_leads.size
  full_name = mailing_campaign_leads[counter][:name]
  names = full_name.split # array with first and last name

  names_counter = 0
  loop do # looping over first and last name to capitlaize both
